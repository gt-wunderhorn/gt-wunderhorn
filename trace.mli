...
(** A path is a linear set of program instructions which will execute as a
    block. *)
type path =
  | Path of P.label * P.label * P.linear_instruction list
  | Assertion of P.label * P.variable

type variable_set = P.Var_set.t

(** What is the label which describes where the path begins? *)
val initial_label : path -> P.label

(** What is the label which describes where the path ends? *)
val terminal_label : path -> P.label
...
(** What are the instructions on the program path? Note that only linear
    ...
    by tracing. *)
val instructions : path -> P.linear_instruction list
...
val critical_variables : t -> P.label -> variable_set
