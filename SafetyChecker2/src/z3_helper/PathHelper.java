...
public class PathHelper {
	...
	public ArrayList<BoolExpr> ConvertToZ3SelfComposition() {
		...
		int __composition_number = 2;
		for(int round = 0; round < 2; round++ ){
		System.out.println("Round " + round + " %^%^%^%^%^%^%^%^%^%^%^%^%^%^%^%^") ;
		for (int i = 0; i < path.size(); i++) {
			Node n = path.get(i);
			System.out.println("n.Stmt  ==== " +  n.getStmt());
			if(n.getIfArryCopy()){
			...
			if (u instanceof IfStmt) {
				System.out.println("u instanceof IfStmt");
			...
			if (u instanceof AssignStmt) {
				...
				if (n.ifDummy()) {
					...
					if (n.getIfInput()) {
						...
						System.out.println("*** leftZ3=" + leftZ3 + " rightZ3=" + rightZ3);
	...
	private Expr CovertRefLikeValue(Value v, boolean IfAssignLeft, int nodeIndex) {
		if (v instanceof PhiExpr) {
			...
			for (int i = 0; i < L1Value.size(); i++) {
				...
				while ((!u1.equals(compareNodeUnit))
						&& (compareNode.predecessor() != null)) {
					count++;
