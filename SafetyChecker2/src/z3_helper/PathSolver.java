...
/*
	public boolean CalPairResult(ArrayList<Node> path1, ArrayList<Node> path2) {
		this.allCoverters.clear();
		PathCoverter Coverter1 = new PathCoverter(0, 1, this.ictx, path1,
				this.forest.getSubFunctionTree(),this.forest);
		PathCoverter Coverter2 = new PathCoverter(1, 2, this.ictx, path2,
				this.forest.getSubFunctionTree(),this.forest);
		this.allCoverters.add(Coverter1);
		this.allCoverters.add(Coverter2);
		PathHelper Helper1 = new PathHelper(Coverter1,
				this.forest.getSubFunctionTree(), path1,this.forest);
		PathHelper Helper2 = new PathHelper(Coverter2,
				this.forest.getSubFunctionTree(), path2,this.forest);
		//Helper1.ConvertToZ3SelfComposition();
		Helper1.ConvertToZ3();
		Helper2.ConvertToZ3();
		...
		BoolExpr InterpolantAndPolicy = this.ictx.mkAnd(inter,
				this.generateAssignAndPolicy());
		System.out.println("InterpolantAndPolicy \n : " + InterpolantAndPolicy);
		Params params = this.ictx.mkParams();
		ComputeInterpolantResult result = this.ictx.ComputeInterpolant(
				InterpolantAndPolicy, params);
...
*/
	public boolean CalPairResult_selfcomposition(ArrayList<Node> path1) {
		this.allCoverters.clear();
		PathCoverter Coverter1 = new PathCoverter(0, 1, this.ictx, path1,
				this.forest.getSubFunctionTree(),this.forest);
//		PathCoverter Coverter2 = new PathCoverter(1, 2, this.ictx, path2,
...
//		BoolExpr inter2 = Coverter2.getInterpolant();
//		BoolExpr inter = this.ictx.mkAnd(inter1, inter2);
		BoolExpr InterpolantAndPolicy = this.ictx.mkAnd(inter1,
				this.generateAssignAndPolicy());
		System.out.println("InterpolantAndPolicy \n : " + InterpolantAndPolicy);
		Params params = this.ictx.mkParams();
		ComputeInterpolantResult result = this.ictx.ComputeInterpolant(
...
//		boolean result = this.CalPairResult(theNewPath, theNewPath);
		boolean result = this.CalPairResult_selfcomposition(theNewPath);
		//boolean compositionResult = this.CalSelfCompositionResult(theNewPath);
		if (result) {
			this.IfCorrect = true;
			this.refine();
			for (int i = 0; i < (size - 1); i++) {
				ArrayList<Node> comparePath = this.paths.get(i);
//				boolean r = this.CalPairResult(comparePath, theNewPath);
				boolean r = this.CalPairResult_selfcomposition(comparePath);
				if (r) {
					this.refine();
					this.IfCorrect = true;
				} else {
...
//		BoolExpr policy = this.ictx.mkEq(output1[0], output1[1]);
